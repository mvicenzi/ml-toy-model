...
class MinkUNetBaseCompress(nn.Module):
    """
    U-ResNet-style sparse model following a trimmed MinkUNet18-style architecture.
    - Initial conv at full resolution
    - Encoder: strided convolutions + residual blocks (2 stages)
    ...
    Encoder:   [B,1,28,28] -> [B,32,28,28] -> [B,32,14,14] -> [B,64,7,7]
    Bottleneck:            [B,64,7,7] -> [B,64,7,7]
    Decoder:   [B,64,7,7] -> [B,32,14,14] -> [B,32,28,28]
    """
    def __init__(self):
        super().__init__()
        ...
        # Stage 1: 7x7 -> 14x14
        self.convtr5 = ConvTrBlock2D(64, 64, kernel_size=2, stride=2)  # Upsample
        self.block6  = ResidualSparseBlock2D(64 + 32, 32)              # Merge skip1, process

        # Stage 2: 14x14 -> 28x28 (full resolution)
        self.convtr7 = ConvTrBlock2D(32, 32, kernel_size=2, stride=2)  # Upsample
        self.block8  = ResidualSparseBlock2D(32 + 32, 32)              # Merge skip0, process

        # ---- Final projection + classification head ----
        self.final = SparseConv2d(32, 32, kernel_size=1, bias=True)    # Feature refinement
        self.head = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(),
            nn.Linear(32, 10),
